 ...
 */
void appendToBufQueue(subscriber *sbcr, uint32 dt){
	intmask	mask;			/* Saved interrupt mask		*/
	mask = disable();
	bufCli * tp = headBuf;
	while(tp->next!=NULL){
...
syscall subscribe(topic16 topic, void (*hndlr)(topic16, uint32)){
	intmask	mask;			/* Saved interrupt mask		*/
	mask = disable();
	int i,flag=-1;
...
syscall publish(topic16 topic, uint32 data){
	...
	kprintf("process : %d is publishing data %u\n", currpid, data);	        
	...
	for(i=0; i<MAX_SUBSCRIBERS;i++){
		if((subs[lower][i].valid==1)&&(subs[lower][i].topicId==topic||upper==0)){
		  appendToBufQueue(&(subs[lower][i]), data);		
		}	
	}
